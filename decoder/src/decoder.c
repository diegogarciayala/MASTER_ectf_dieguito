...
void aes_cmac(const uint8_t *key, const uint8_t *data, uint32_t data_len, uint8_t *out_mac) {
    ...
    // Inicializar estructura CMAC con la clave
    if (wc_CmacInit(&cmac, key, 16, AES_BLOCK_SIZE) != 0) {
        memset(out_mac, 0, 16);
        return;
    }
    
    // Actualizar con los datos
    if (wc_CmacUpdate(&cmac, data, data_len) != 0) {
        memset(out_mac, 0, 16);
        return;
    }
    
    // Finalizar y obtener el MAC
    word32 outLen = 16;
    if (wc_CmacFinal(&cmac, out_mac, &outLen) != 0) {
        memset(out_mac, 0, 16);
        return;
    }
}
