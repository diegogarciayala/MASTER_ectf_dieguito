 ...
 *          Adaptado para que el formato final (tamaños, estructura de mensajes)
 *          sea igual al ejemplo original, pero integrando cifrado (AES-CTR),
 *          derivación de claves vía CMAC y verificación de suscripción.
 *
 *  El paquete final tiene 152 bytes, compuesto por:
 *    - Header: 20 bytes (<I I I Q>)  --> seq (4), channel (4), encoder_id (4), timestamp (8)
 *    - Suscripción: 52 bytes (36 bytes de datos + 16 bytes de CMAC)
 *    - Cifrado: 80 bytes (Frame de 64 bytes + Trailer de 16 bytes)
 *
 * @date    2025
 */
 ...
 #define CIPHER_SIZE      (FRAME_SIZE + TRAILER_SIZE)         // 80 bytes
 
 #define PACKET_MIN_SIZE  (HEADER_SIZE + SUBS_TOTAL_SIZE + CIPHER_SIZE)  // 20+52+80=152 bytes
 ...
 typedef struct {
     ...
     uint8_t data[FRAME_SIZE];  // 64 bytes de frame
 } frame_packet_t;
 ...
 /* =================== CRYPTO FUNCTIONS (CMAC, AES-CTR) =================== */
 static uint8_t G_K_MASTER[16];    // Clave maestra (dummy)
 static uint8_t g_channel_key[32];   // Clave del canal (dummy)
 
 // Para este ejemplo, load_secure_keys() fija las claves de forma fija.
 // En un sistema real se debería leer un archivo JSON (por ejemplo, montado en /global.secrets)
 // y extraer la clave correspondiente al canal.
 ...
 /* AES-CTR para cifrado; se asume que nonce tiene 16 bytes */
 ...
 static int secure_process_packet(const uint8_t* packet, size_t packet_len,
     ...
     printf("[decoder] Procesando paquete seguro (%zu bytes)\n", packet_len);
     fflush(stdout);
     ...
     /* Verificar CMAC de la sección de suscripción */
     ...
     /* Derivar dynamic_key usando CMAC con "K1-Derivation" */
     uint8_t K1[16];
     ...
     /* Descifrar la porción de frame cifrado (80 bytes) */
 ...
 int decode(pkt_len_t pkt_len, uint8_t *encrypted_buf) {
 ...
 int list_channels(void) {
     ...
     // Aquí se empaqueta la respuesta. (En el ejemplo original se enviaba
     // sizeof(n_channels) + (sizeof(channel_info_t) * n_channels) bytes)
 ...
 int update_subscription(pkt_len_t pkt_len, subscription_update_packet_t *update) {
     ...
     if (i == MAX_CHANNEL_COUNT) {
         STATUS_LED_RED();
         print_error("Máximo número de suscripciones alcanzado\n");
         return -1;
     }
 ...
 void init(void) {
     ...
     if (decoder_status.first_boot != FLASH_FIRST_BOOT) {
         ...
         for (int i = 0; i < MAX_CHANNEL_COUNT; i++){
             subscription[i].start_timestamp = 0xFFFFFFFFFFFFFFFF;
             subscription[i].end_timestamp = 0xFFFFFFFFFFFFFFFF;
     ...
     if (load_secure_keys() != 0) {
         STATUS_LED_ERROR();
         print_error("Error al cargar claves seguras\n");
         while (1);
     }
 }
 ...
 int main(void) {
     ...
     while (1) {
         ...
         if (result < 0) {
             STATUS_LED_ERROR();
             print_error("Error al recibir comando desde el host\n");
             continue;
         }
         switch (cmd) {
             case LIST_MSG:
